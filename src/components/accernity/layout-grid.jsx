...
//   return (
//     <div className="w-full h-full py-4 md:py-10 grid grid-cols-1 md:grid-cols-2 max-w-7xl mx-auto gap-4 relative">
//       {cards.map((card, i) => (
...
export const LayoutGrid = ({ cards }) => {
  return (
    <div className="w-full h-full py-4 md:py-10 grid grid-cols-1 md:grid-cols-2 max-w-7xl mx-auto gap-4">
      {cards.map((card, i) => (
        <div key={i} className={cn(card.className)}>
          <motion.div
            className="bg-[#141c27] rounded-xl overflow-hidden w-full shadow-md hover:shadow-xl transition duration-300"
          >
            {card.content}
          </motion.div>
        </div>
