import React from "react";
import { DotLottieReact } from "@lottiefiles/dotlottie-react";

const Skills = () => {
  const skills = [
    {
      name: "React js",
      strength: "80%",
      logo: "/React.png",
    },
    {
      name: "Tailwind CSS",
      strength: "85%",
      logo: "/tailwind.svg",
    },
    {
      name: "HTML",
      strength: "90%",
      logo: "/html.png",
    },
    {
      name: "CSS",
      strength: "85%",
      logo: "/css.webp",
    },
    {
      name: "JavaScript",
      strength: "70%",
      logo: "/JS.png",
    },
    {
      name: "Node.js",
      strength: "65%",
      logo: "/NodeJs.svg",
    },
    {
      name: "Express.js",
      strength: "70%",
      logo: "/ExpressJs.png",
    },
    {
      name: "Bootstrap",
      strength: "70%",
      logo: "/Bootstrap.svg",
    },
    {
      name: "SEO",
      strength: "65%",
      logo: "/seo.jpg",
    },
    {
      name: "Redux State Management",
      strength: "75%",
      logo: "/Redux.svg",
    },
  ];

  return (
    <div className="bg-[#141c27] min-h-screen px-3 md:px-[90px] pt-[50px] pb-[50px]">
      <h1 className="text-[#55e6a5] text-2xl font-lg relative before:absolute before:h-[2px] before:w-[100px] before:bg-[#55e6a5] before:top-4 before:left-[-100px] lg:mx-5 pl-[30px] mb-[50px]">
        Hands-on Experience In
      </h1>
      <div className="flex flex-col md:flex-row w-full items-center justify-between gap-6 md:gap-10">
        {/* Left Column: First 5 Skills */}
        <div className="w-full md:w-[35%] flex flex-col gap-3">
          {skills.slice(0, 5).map((skill, index) => (
            <div
              key={index}
              className="flex h-[100px] items-center gap-4 bg-[#1b2735]/80 backdrop-blur-md p-4 rounded-xl shadow-md hover:shadow-lg transition duration-300 group border border-[#55e6a5]/20"
            >
              <img
                src={skill.logo}
                alt={`${skill.name} logo`}
                className="h-16 w-16 rounded-md object-contain border border-[#55e6a5] p-1 bg-black/30 group-hover:scale-105 transition duration-300"
              />
              <div className="flex flex-col w-full">
                <div className="text-white text-lg font-semibold">
                  {skill.name}
                </div>
                {/* Uncomment if you want to show strength */}
                {/* <div className="text-sm text-[#55e6a5] font-medium">
                  {skill.strength}
                </div> */}
              </div>
            </div>
          ))}
        </div>

        {/* Center: Lottie Animation */}
        <div className="w-full md:w-[45%] flex justify-center items-center">
          <div className=" h-[300px] w-[300px] md:w-[600px] md:h-[550px] lg:bg-[#1b2735]/80 lg:backdrop-blur-md lg:border lg:border-[#55e6a5]/20 p-4">
            <DotLottieReact
              src="https://lottie.host/6b346d52-2ac0-49b6-8ae6-6465be300f31/Q7gxlbra8i.lottie"
              loop
              autoplay
            />
          </div>
        </div>

        {/* Right Column: Last 5 Skills */}
        <div className="w-full md:w-[35%] flex flex-col gap-3">
          {skills.slice(5, 10).map((skill, index) => (
            <div
              key={index}
              className="flex h-[100px] items-center gap-4 bg-[#1b2735]/80 backdrop-blur-md p-4 rounded-xl shadow-md hover:shadow-lg transition duration-300 group border border-[#55e6a5]/20"
            >
              <img
                src={skill.logo}
                alt={`${skill.name} logo`}
                className="h-16 w-16 rounded-md object-contain border border-[#55e6a5] p-1 bg-black/30 group-hover:scale-105 transition duration-300"
              />
              <div className="flex flex-col w-full">
                <div className="text-white text-lg font-semibold">
                  {skill.name}
                </div>
                {/* Uncomment if you want to show strength */}
                {/* <div className="text-sm text-[#55e6a5] font-medium">
                  {skill.strength}
                </div> */}
              </div>
            </div>
          ))}
        </div>
      </div>
    </div>
  );
};

export default Skills;
